...
職責：
- 深度分析 Excel/Word 表格結構
- 從真實表單自動建立 InspectionTemplate
- AI 智慧欄位映射
- 精準欄位映射（基於 photo_task_bindings）
"""
...
from app.services.form_utils import (
    ...
    is_section_header, is_non_field_item,
)
...
class FormAnalysisService:
    ...
    async def create_template_from_file(
        ...
        if file_type == 'xlsx':
            ...
            raw_text = await self._extract_excel_text(file_content)
        elif file_type == 'docx':
            field_map = await self._deep_analyze_word(file_content)
            raw_text = await self._extract_word_text(file_content)
        else:
...
只回應 JSON，不要其他文字。"""
    ...
    async def _deep_analyze_word(self, content: bytes) -> list[dict]:
        ...
        for para_idx, para in enumerate(doc.paragraphs):
            ...
            if is_field_label(text) or '____' in text or '＿＿' in text:
